...
read -r -d '' USAGE <<'EOF'
Query GDC to obtain sample and aliquot details associated with a given TCGA case

Usage:
  get_aliquots_v2.sh [options] CASE
...
OUTLINE=$(echo $R | $PYTHON src/parse_aliquot_TCGA.py -c $CASE )
test_exit_status
