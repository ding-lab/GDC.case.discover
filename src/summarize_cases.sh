...
# Get counts for (tumor, normal, tissue) x (WGS.hg19, WXS.hg19, WGS.hg38, WXS.hg38, RNA-Seq, miRNA-Seq, MethArray)
...
# Note that other rare sample types (Nbc, Tbm, Tpb) will not be counted.  These should be merged with N and T
    ...
    function count_entries {
        ...
        awk -v c=$CASE -v es="$ES" -v st=$ST -v ref=$REF 'BEGIN{FS="\t";OFS="\t"}{if ( ($2 == c) && ($4 == es) && ($5 == st) && ($13 == ref)) print}' $CATALOG | wc -l
    }
    ...
    METH_T=$(count_entries $CASE "Methylation Array" tumor NA)
    METH_N=$(count_entries $CASE "Methylation Array" blood_normal NA)
    METH_A=$(count_entries $CASE "Methylation Array" tissue_normal NA)
